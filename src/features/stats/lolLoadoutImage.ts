...
export async function buildLoadoutImage(participant: MatchParticipantDto): Promise<Buffer> {
  const icons = await loadParticipantIcons(participant);
  const [item0, item1, item2, item3, item4, item5, trinket] = icons.items;
  ...
  drawIcon(ctx, icons.spell1, 0, 0);
  drawIcon(ctx, icons.spell2, ICON_SIZE + COL_GAP, 0);
  drawIcon(ctx, icons.keystone, 0, ICON_SIZE + ROW_GAP);
  drawIcon(ctx, icons.secondary, ICON_SIZE + COL_GAP, ICON_SIZE + ROW_GAP);
